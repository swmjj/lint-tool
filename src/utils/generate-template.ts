...
export default (cwd: string, data: Record<string, any>, vscode?: boolean) => {
  ...
  for (const name of templates) {
    ...
    const pathName = filepath.includes('pkg.config.js') ? filepath.replace('pkg.config.js', `${PKG_NAME}.config.js`) : filepath;

    fs.outputFileSync(pathName, content, 'utf8');
  }
};
