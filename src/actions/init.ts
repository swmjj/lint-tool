...
export default async (options: InitOptions) => {
  ...
  if (!pkg.scripts[`${PKG_NAME}-scan`]) {
    pkg.scripts[`${PKG_NAME}-scan`] = `npx ${PKG_NAME} scan`;
  }
  if (!pkg.scripts[`${PKG_NAME}-fix`]) {
    pkg.scripts[`${PKG_NAME}-fix`] = `npx ${PKG_NAME} fix`;
  }

  if (!pkg.scripts.changelog) {
    pkg.scripts.changelog = 'npx conventional-changelog -p angular -i CHANGELOG.md -s';
  }
  ...
  // 添加changeLog 配置
  if (!pkg.scripts.changelog) {
    pkg.scripts.changelog = 'npx standard-version';
  }
  ...
  pkg.husky.hooks['pre-commit'] = `npx ${PKG_NAME} commit-file-scan`;
  // pkg.husky.hooks['commit-msg'] = `npx ${PKG_NAME} commit-msg-scan`;
  pkg.husky.hooks['commit-msg'] = 'npx commitlint -e $HUSKY_GIT_PARAMS';
